...
#*******************************************************************************
# Sensor messages
pub_gps = rospy.Publisher('gps_2', NavSatFix)
#pub_imu = rospy.Publisher('imu', Imu)
pub_rc = rospy.Publisher('rc_2', roscopter.msg.RC)
pub_state = rospy.Publisher('state_2', roscopter.msg.State)
pub_vfr_hud = rospy.Publisher('vfr_hud_2', roscopter.msg.VFR_HUD)
pub_attitude = rospy.Publisher('attitude_2', roscopter.msg.Attitude)
pub_raw_imu =  rospy.Publisher('raw_imu_2', roscopter.msg.Mavlink_RAW_IMU)
pub_status = rospy.Publisher('status_2', roscopter.msg.Status)
pub_filtered_pos = rospy.Publisher('filtered_pos_2', roscopter.msg.FilteredPosition)
pub_control_output = rospy.Publisher('controller_output_2', roscopter.msg.ControllerOutput)
pub_current_mission = rospy.Publisher('current_mission_2', roscopter.msg.CurrentMission)
pub_mission_item = rospy.Publisher('mission_item_2', roscopter.msg.MissionItem)

##******************************************************************************
# Services for APM Commands
#*******************************************************************************
# Allow for commands such as Arm, Disarm, Launch, Land, etc.
rospy.Service("command_2", roscopter.srv.APMCommand, command_cb)
...
if opts.enable_rc_control:
    rospy.Subscriber("send_rc_2", roscopter.msg.RC , send_rc_cb)
